...
- El ciclo de la rama de funcionalidades local ejecuta compilaciones, pruebas y Docker Compose contra raíces fixture y PostgreSQL desechables; nunca monta la biblioteca musical de producción.
- La compilación, las pruebas y la verificación repetibles de Docker Compose en WSL son la puerta de entrega. Después se construyen localmente imágenes inmutables `linux/amd64` desde el commit exacto con una etiqueta derivada explícitamente del commit; nunca se usa una etiqueta flotante `latest`.
- El objetivo actual de prueba del NAS en la LAN es `192.168.68.100`, pero el host, el usuario SSH, la ruta de despliegue y la etiqueta de imagen siguen siendo configurables y las credenciales nunca se guardan en el repositorio.
- El despliegue de prueba en el NAS usa una definición de Compose versionada, secretos/configuración exclusivos del NAS y raíces de prueba desechables o representativas antes de montar cualquier biblioteca real.
- Una operación WSL parametrizada y documentada transfiere directamente al NAS por SSH las imágenes con etiqueta exacta, las carga sin requerir un registry, hace una copia de seguridad de PostgreSQL, ejecuta migraciones explícitas, inicia el stack de prueba de Compose versionado y realiza comprobaciones de salud y smoke con fallos visibles.
- El rollback selecciona la etiqueta de imagen inmutable anterior. La base de datos se restaura desde la copia previa a la migración solo cuando la compatibilidad de la migración lo requiere; la degradación del esquema nunca es implícita.
